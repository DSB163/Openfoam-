...
:
    ...
    currentDriven_(false),
    totalCurrent_(0.0)
{
    // Validate size consistency
    if (!this->coeffDict().found("current"))
    {
        // Frequency-driven mode: validate frequencies
    ...
    // else: current-driven mode will set iF_ below
    ...
    {
        currentDriven_ = true;
        totalCurrent_ = readScalar(this->coeffDict().lookup("current"));

        // Validate eV size
        if(eV_.size() != injectionSpeciesList_.size())
        {
            FatalErrorInFunction
                    << "not the same number of entries in eV, injectionTypeIds" << nl
                    << abort(FatalError);
        }

        // Single species: all current goes to species 0
        iF_.setSize(injectionSpeciesList_.size());
        forAll(iF_, i)
        {
            label typeId = injectionSpeciesTypeIds_[i];
            scalar q = cloud.constProps(typeId).charge();
            
            if (q == 0.0)
            {
                FatalErrorInFunction
                    << "Species " << injectionSpeciesList_[i] 
                    << " has zero charge, cannot use current-driven mode" << nl
                    << abort(FatalError);
            }

            scalar I_i = totalCurrent_ / injectionSpeciesList_.size();  // equal split
            iF_[i] = I_i / q;  // N_dot = I/q [particles/s]

            Info << "    Current-driven: " << injectionSpeciesList_[i]
                 << " I=" << I_i << " A, q=" << q << " C, N_dot=" << iF_[i] << " s^-1" << endl;
        }

        Info << "[" << typeName << "] Total current I=" << totalCurrent_ 
             << " A, total N_dot=" << sum(iF_) << " particles/s" << endl;
    }
    else
    {
        // Frequency-driven mode: read and validate frequencies
        iF_ = scalarList(this->coeffDict().lookup("frequencies"));
...
void Foam::SimpleInjection<CloudType>::injection()
{
      ...
      // Log current-driven mode
      if(currentDriven_)
      {
          Info<< "[" << typeName << "] Current-driven: I=" 
              << totalCurrent_ << " A" << endl;
      }
